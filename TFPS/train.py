...
def train_model(model, x_train, y_train, name, scats, junction, config):
    ...
    """
    model.compile(loss="mse", optimizer="nadam", metrics=['mape', 'mae', 'mse'])
...
def train_seas(models, x_train, y_train, name, scats, junction, config):
    ...
    for i in range(len(models) - 1):
        ...
        m = models[i]
        m.compile(loss="mse", optimizer="nadam", metrics=['mape', 'mae', 'mse'])
...
def main(argv):
    ...
    parser.add_argument(
        "--model",
        default="lstm,gru,feedfwd,deepfeedfwd",
        help="Model to train.")
